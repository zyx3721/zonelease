...
try {
  ...
  if (-not (Test-Administrator)) {
    throw "Please run this script as Administrator. DHCP Server PowerShell operations require elevated privileges."
  }
  ...
  if ($LegacySource) {
    ...
    if (-not (Test-Path -LiteralPath $legacyScript)) {
      throw "Legacy DHCP agent script not found. Expected legacy\source-agent.ps1 or source-agent.ps1 in $scriptRoot."
    }
    ...
    Write-Host "Starting ZoneLease DHCP Legacy Agent..."
  ...
  if (Test-Blank $Exe) {
    $candidates = @(
      (Join-Path $scriptRoot "zonelease-dhcp-agent.exe"),
      (Join-Path $scriptRoot "dhcp-agent.exe"),
  ...
  if (-not (Test-Path -LiteralPath $Exe)) {
    throw "DHCP agent executable not found. Expected zonelease-dhcp-agent.exe, dhcp-agent.exe, or agent.exe in $scriptRoot."
  }
  ...
  Write-Host "Starting ZoneLease DHCP Agent..."
  Write-Host "Executable: $Exe"
  & $Exe
  exit $LASTEXITCODE
}
catch {
  Write-Host "Start DHCP agent failed: $($_.Exception.Message)" -ForegroundColor Red
  exit 1
}
