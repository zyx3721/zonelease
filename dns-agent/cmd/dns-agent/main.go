...
import (
	...
	"zonelease/dns-agent/internal/server"
)
...
func main() {
	...
	if err != nil {
		_, _ = os.Stderr.WriteString("failed to initialize dns agent log: " + err.Error() + "\n")
		os.Exit(1)
	}
	defer logFile.Close()
	slog.SetDefault(logger)

	app := server.New(cfg, dns.NewPowerShellProviderWithTimeout(time.Duration(cfg.PowerShellTimeoutSeconds)*time.Second), logger)
	httpServer := &http.Server{Addr: cfg.Addr(), Handler: app.Routes(), ReadHeaderTimeout: 10 * time.Second}

	go func() {
		logger.Info("dns agent listening", "addr", cfg.Addr(), "logPath", cfg.LogPath, "powershellTimeoutSeconds", cfg.PowerShellTimeoutSeconds)
		if err := httpServer.ListenAndServe(); err != nil && !errors.Is(err, http.ErrServerClosed) {
			logger.Error("dns agent stopped unexpectedly", "error", err)
			os.Exit(1)
		}
	}()
	...
	if err := httpServer.Shutdown(ctx); err != nil {
		logger.Error("dns agent shutdown failed", "error", err)
		os.Exit(1)
	}
}
