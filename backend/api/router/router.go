...
func New(cfg config.Config, store *repository.Store, authService *auth.Service, realtimeService *realtime.Service, syncService *syncsvc.Service, logger *slog.Logger) http.Handler {
	...
	mux.HandleFunc("GET /api/auth/wecom/authorize", r.wecomAuthorize)
	mux.HandleFunc("GET /api/auth/wecom/callback", r.wecomCallback)
	mux.HandleFunc("POST /api/auth/wecom/exchange", r.wecomExchange)
	mux.HandleFunc("POST /api/auth/wecom/login", r.wecomLogin)
	...
	protected.HandleFunc("GET /api/auth/wecom/bind-url", r.wecomBindURL)
	protected.HandleFunc("POST /api/auth/wecom/bind", r.wecomBind)
	protected.HandleFunc("POST /api/auth/wecom/unbind", r.wecomUnbind)
	...
	mux.Handle("GET /api/auth/wecom/bind-url", r.requireAuth(protected))
	mux.Handle("POST /api/auth/wecom/bind", r.requireAuth(protected))
	mux.Handle("POST /api/auth/wecom/unbind", r.requireAuth(protected))
