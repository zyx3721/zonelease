...
func (s *Store) FindSession(ctx context.Context, token string) (domain.User, time.Time, time.Time, error) {
	var user domain.User
	var expiresAt time.Time
	var lastSeenAt time.Time
	err := s.pool.QueryRow(ctx, `
		SELECT u.id::text, u.username, u.email, u.display_name, u.role, COALESCE(NULLIF(u.source, ''), 'local'), u.disabled, u.last_login_at, u.created_at, u.updated_at, s.expires_at, s.last_seen_at
		FROM sessions s JOIN users u ON u.id = s.user_id
		WHERE s.token_hash=$1
	`, HashToken(token)).Scan(&user.ID, &user.Username, &user.Email, &user.DisplayName, &user.Role, &user.Source, &user.Disabled, &user.LastLoginAt, &user.CreatedAt, &user.UpdatedAt, &expiresAt, &lastSeenAt)
	...
	return users[0], expiresAt, lastSeenAt, nil
}
