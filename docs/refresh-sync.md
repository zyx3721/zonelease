...
- 短期运行态缓存 key：
  ...
  - `zonelease:runtime:agent-sync:<serverId>`：标记指定 Agent 正在执行全量同步、自动全量同步或单 Agent 同步，TTL 为 Agent 全量同步超时时间加 1 分钟；自动健康检查读取该 key 后会跳过正在同步的 Agent，DNS / DHCP 管理操作、手动 DNS 区域 / DHCP 作用域刷新或单 Agent 同步读取该 key 后会提示当前 Agent 正在同步并拒绝本次操作。
...
Agent 测试和 DNS / DHCP 管理写入类操作的整体超时时间由「系统配置 / 基础配置 / Agent 判定」保存到 PostgreSQL 后读取。
