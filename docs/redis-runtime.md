...
- Agent 同步运行态标记 TTL 为 Agent 全量同步超时时间加 `1` 分钟，任务结束后会主动删除。
...
| `zonelease:runtime:agent-sync:<serverId>` | String，JSON | Agent 全量同步超时时间加 1 分钟，任务结束后主动删除 | 指定 Agent 同步任务开始时写入，结束时删除 | 自动健康检查、DNS / DHCP 管理操作、手动 DNS 区域 / DHCP 作用域刷新和单 Agent 同步执行前读取 |
...
| key | 类型 | 过期策略 | 用途 |
| :-: | :--: | :------: | :--: |
| `zonelease:lock:agent-health-check` | String，随机 token | Agent 操作超时乘以检查目标数量，再加 1 分钟 | 避免多实例同时执行自动健康检查 |
| `zonelease:lock:refresh:scheduled-dns` | String，随机 token | Agent 全量同步超时加 1 分钟 | 避免多实例同时创建 DNS 定时全量刷新任务 |
| `zonelease:lock:refresh:scheduled-dhcp` | String，随机 token | Agent 全量同步超时加 1 分钟 | 避免多实例同时创建 DHCP 定时全量刷新任务 |
| `zonelease:lock:refresh:all` | String，随机 token | Agent 全量同步超时加 1 分钟 | 避免全量刷新任务并发执行 |
| `zonelease:lock:refresh:dns-all` | String，随机 token | Agent 全量同步超时加 1 分钟 | 避免 DNS 全量刷新任务并发执行 |
| `zonelease:lock:refresh:dhcp-all` | String，随机 token | Agent 全量同步超时加 1 分钟 | 避免 DHCP 全量刷新任务并发执行 |
| `zonelease:lock:agent-sync:<serverId>` | String，随机 token | Agent 全量同步超时时间加 1 分钟 | 避免全量同步和单 Agent 同步并发访问同一 Agent |
| `zonelease:lock:refresh:server:<serverId>` | String，随机 token | Agent 全量同步超时加 1 分钟 | 避免同一 Agent 同步任务并发执行 |
| `zonelease:lock:refresh:dns-zone:<serverId>:<zoneName>` | String，随机 token | Agent 全量同步超时加 1 分钟 | 避免同一 DNS 区域刷新任务并发执行 |
| `zonelease:lock:refresh:dhcp-scope:<serverId>:<scopeExternalId>` | String，随机 token | Agent 全量同步超时加 1 分钟 | 避免同一 DHCP 作用域刷新任务并发执行 |
