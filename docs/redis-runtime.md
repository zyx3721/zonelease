...
| `RUNTIME_DEEP_SYNC_INTERVAL` | `0`            | 定时全量同步间隔；启用后会使用 Redis 锁避免多实例重复排队    |
...
- `type`：事件类型，例如 `runtime.refresh.all`、`runtime.refresh.server`、`runtime.refresh.dns.zone`、`runtime.refresh.dhcp.scope` 或 `runtime.updated`。
- `taskId`：刷新任务 ID。
...
| `zonelease:lock:refresh:scheduled-all` | String，随机 token | 定时全量同步间隔 | 避免多实例同时创建定时全量刷新任务 |
