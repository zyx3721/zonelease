...
7. 后端通过 Redis Pub/Sub 实时发布，并写入 Redis Stream 支持回放，前端收到事件后重新读取数据库快照。
...
DNS 记录能力分为同步展示、前端创建和编辑 / 删除三个边界。DNS Agent 记录采集支持以下类型，能解析出有效值的记录会作为快照展示：
...
前端新建记录入口当前只开放 `A` 和 `CNAME`。新建记录前，后端会直接基于 PostgreSQL 中的当前区域记录快照做冲突校验：

- `A` 按 Windows DNS“新建主机”语义提交为 `A` 记录。
...
DNS 区域同步和全量同步中的 DNS 记录采集阶段由「系统配置 / 基础配置 / Agent 判定」中的 Agent 全量同步超时控制，默认 `300` 秒，可配置 `60` 到 `600` 秒。
...
后端未读取到入库配置时，DNS 区域并发使用默认值 `3`，Agent 连接超时使用默认值 `5` 秒，Agent 操作超时使用默认值 `20` 秒，Agent 全量同步超时使用默认值 `300` 秒。
...
当 `DNS_AGENT_ALLOW_ANONYMOUS=false` 且 `DNS_AGENT_API_KEY` 非空时，业务接口需要携带：

```text
X-API-Key: <DNS_AGENT_API_KEY>
```

如果 Agent 显式开启匿名访问，或 `DNS_AGENT_API_KEY` 留空，则不会校验业务接口 API Key。生产环境应配置非空强随机 API Key，并保持匿名访问关闭。
