> 本文件是 ZoneLease 完整手册，由项目根 README 迁移而来，保留全部部署步骤、使用说明、API 清单与版本历史；新版精简说明见根目录 [README.md](../README.md) 与 [README.en.md](../README.en.md)。
...
- **用户认证**：默认管理员初始化、登录、注销、当前用户查询、修改密码、找回密码验证码流程，以及企业微信扫码登录与账号绑定 / 解绑（支持直连企业微信和统一认证中心两种接入模式，仅允许已绑定平台用户的企业微信账号登录）。
...
Redis 只承载短期运行态能力，包括 SSE 刷新事件、最近事件回放、刷新任务进度、Agent 健康检查缓存、通知未读数缓存和短期分布式锁。接口读取仍以 PostgreSQL 为准；刷新链路、锁边界和 Redis key 细节见 [docs/refresh-sync.md](refresh-sync.md) 与 [docs/redis-runtime.md](redis-runtime.md)。
...
|                               文档                               |                                     适用场景                                     |
| :--------------------------------------------------------------: | :------------------------------------------------------------------------------: |
|         [docs/dns-management.md](dns-management.md)         | 查看 DNS 区域、DNS 记录、Agent 接口、数据库快照、区域刷新和记录变更后的同步链路  |
|        [docs/dhcp-management.md](dhcp-management.md)        |      查看 DHCP 作用域、租约、保留地址、Agent 接口、数据库快照和当前操作边界      |
|           [docs/refresh-sync.md](refresh-sync.md)           | 查看页面刷新、手动全量刷新、定时全量刷新、区域刷新、Redis/SSE 事件和环境变量边界 |
|          [docs/redis-runtime.md](redis-runtime.md)          |        查看 Redis key、Stream、Pub/Sub、运行态缓存、分布式锁和运维查看方式        |
| [docs/operation-log-coverage.md](operation-log-coverage.md) |               查看刷新任务、操作审计和通知中心的写入边界与覆盖范围               |
| [docs/frontend-select-dropdown-placement.md](frontend-select-dropdown-placement.md) |       查看前端 Select 与手写 listbox 控件的下拉定位、层级和裁切规避规则       |
...
前端 SSR 首屏会从后端读取品牌配置（站点名称与图标）并直出到 HTML，保证刷新时浏览器标签页不闪现默认品牌：

- SSR 直连后端与开发代理共用同一变量 `VITE_API_BASE_URL` ，后端地址非默认值时在 `.env` 文件配置一次即可
- 生产部署可用 `ZONELEASE_SSR_API_ORIGIN` 单独指定 SSR 直连后端地址（优先读取运行时环境变量，其次读取进程入口目录向上各层的 `.env` 文件），无需重新构建前端
- 未配置时默认通过 `http://127.0.0.1:8080` 访问后端，与后端同机部署时无需额外配置
- 后端不可达时 SSR 自动回退默认品牌，页面加载完成后客户端仍会拉取最新品牌展示
...
DNS 区域、记录字段、区域级刷新、Agent 接口和数据库快照策略详情见 [docs/dns-management.md](dns-management.md)。
...
DHCP 作用域、排除范围、租约、保留地址字段、Agent 接口、数据库快照策略和当前操作边界详情见 [docs/dhcp-management.md](dhcp-management.md)。
...
全量同步、DNS 区域和 DHCP 作用域采集并发，以及操作后局部刷新等待时间从「系统配置 / 基础配置 / 同步参数」读取并入库保存。Agent 连接测试、自动健康检查和同步前 `/health` 检查默认 `5` 秒超时；DNS / DHCP 管理操作超时时间默认 `20` 秒；全量同步、单 Agent 同步和局部同步的资源采集超时时间默认 `300` 秒。各刷新入口是否访问 Agent、刷新范围、任务类型、Redis/SSE 事件和配置边界详情见 [docs/refresh-sync.md](refresh-sync.md)。

## 6.6 系统配置

「系统配置」页面参考配置中心布局，包含：

- **基础配置**：维护站点名称、登录展示、控制台品牌、找回密码安全时效、企业微信扫码有效期、同步并发、操作后刷新等待、Agent 离线判定次数、Agent 连接超时、Agent 操作超时、Agent 全量同步超时、自动健康检查间隔和自动检查并发。
- **用户配置**：维护平台用户、用户群组和角色权限。启用 AD/LDAP 后，外部账号也必须先在用户中创建并启用。
- **认证配置**：维护 AD/LDAP 目录认证和企业微信认证；企业微信支持「直连企业微信」和「统一认证中心」两种卡片式接入模式切换，面板内提供「认证方式」「应用配置」分组、字段悬浮提示和配置步骤指引；与 itdb-new 一致，两种模式的配置均会保留，切换认证方式即切换使用哪一组配置，不会丢失另一模式的凭据；「清空配置」在保存时执行，保存后该认证方式停用且配置整体置空；企微账号与平台用户的绑定关系保存在 `user_wecom_bindings` 表，登录、当前用户和用户管理响应都会携带 `wecomBound` 绑定状态。
...
- DNS、DHCP 和刷新同步的详细边界分别见 [docs/dns-management.md](dns-management.md)、[docs/dhcp-management.md](dhcp-management.md) 和 [docs/refresh-sync.md](refresh-sync.md)。
...
+ts)` 签名调用统一认证中心 `/api/verify` 一次性换取 userid
  - 按 userid 匹配已绑定的平台用户并建立会话，签发 60 秒一次性登录票据后 302 回前端登录页
  - 账号未绑定时携带 `wecomError=user_not_bound` 回跳；绑定用户被禁用时携带 `wecomError=user_not_provisioned` 回跳
...
- `POST /api/auth/wecom/bind` - 绑定企业微信
  - 需要登录；直连模式提交 `code` 与绑定 `state`，统一认证中心模式提交 `ticket`
  - 该企微账号已绑定其他用户时返回 409；绑定成功写入 `Bound wecom` 审计记录
- `POST /api/auth/wecom/unbind` - 解绑企业微信
  - 需要登录；未绑定时幂等返回 `bound=false`；解绑成功写入 `Unbound wecom` 审计记录
...
DHCP 字段、Agent 接口、数据库快照策略和当前操作边界详见 [docs/dhcp-management.md](dhcp-management.md)。
...
- `POST /api/dns/zones/{id}/refresh` - 刷新指定 DNS 区域记录
  ...
  - DNS 区域和记录快照策略详见 [docs/dns-management.md](dns-management.md)
...
- `GET /api/public/base` - 获取公开系统基础配置
  ...
  - 登录页、忘记密码页、启动页和控制台布局会读取该接口同步品牌展示
...
- `PUT /api/settings/auth-providers/{id}` - 保存认证配置
  ...
  - 密码留空且已有配置时保留原值：`ldap` 保留绑定密码，`wecom` 保留应用 Secret 与应用对接密钥
  - 企业微信两种模式的配置均会保留，`mode` 仅决定当前启用的一组
  - `clearConfig` 为 `true` 时清空该认证配置并停用（请求中的 `enabled` 与 `config` 被忽略），审计 metadata 记录 `cleared=true`
- `POST /api/settings/auth-providers/{id}/test` - 测试认证配置
  - `{id}` 当前仅支持 `ldap`，返回 `matchedUsers` 表示 LDAP 搜索匹配用户数量
...
- `POST /api/refresh` - 创建刷新任务并发布 SSE 事件
  ...
  - 刷新范围、任务状态、Redis/SSE 事件和定时刷新规则详见 [docs/refresh-sync.md](refresh-sync.md)
...
任务、审计和通知中心的详细写入边界见 [docs/operation-log-coverage.md](operation-log-coverage.md)。
...
新增企业微信扫码认证（直连与统一认证中心两种接入模式）与企微账号绑定 / 解绑体系，认证相关界面与交互对照 itdb-new 全面优化；根 README 重整并新增英文版，完整手册迁移至 `docs/manual.md`，部署文档新增 Release 二进制部署流程。详见 [verchanglog/v1.1.0.md](../verchanglog/v1.1.0.md)。
