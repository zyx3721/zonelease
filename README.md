...
- **认证配置**：维护 AD/LDAP 目录认证和企业微信认证；企业微信支持「直连企业微信」和「统一认证中心」两种卡片式接入模式切换，面板内提供「认证方式」「应用配置」分组和字段悬浮提示，模式切换保存后自动清理另一模式遗留凭据，并支持连接测试；企微账号与平台用户的绑定关系保存在 `user_wecom_bindings` 表，登录、当前用户和用户管理响应都会携带 `wecomBound` 绑定状态。
