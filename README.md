...
启用企业微信认证后，登录页会出现「企业微信登录」入口；已登录用户可在右上角用户菜单使用「绑定企微 / 解绑企微」。企业微信账号必须先绑定平台用户才能扫码登录，绑定冲突（同一企微账号绑定多个用户）会被拒绝并提示。接入模式分为两种：
...
- **认证配置**：维护 AD/LDAP 目录认证和企业微信认证；企业微信支持「直连企业微信」和「统一认证中心」两种接入模式，模式切换保存后自动清理另一模式遗留凭据，并支持连接测试；企微账号与平台用户的绑定关系保存在 `user_wecom_bindings` 表，登录、当前用户和用户管理响应都会携带 `wecomBound` 绑定状态。
