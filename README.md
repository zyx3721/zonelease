...
>
> - 兼容模式会自动读取 `dhcp-agent/.env` 中的端口、API Key、匿名访问和日志路径配置。
> - JSON 请求体默认按 UTF-8 解码，避免中文名称和描述被 Windows 默认代码页误读为乱码。
> - legacy Agent 通过 `netsh dhcp server` 执行 DHCP 作用域、租约和保留地址操作。
> - legacy 作用域名称和描述会优先从全局 `dump` 的 `add scope` 行解析，描述为空时保持为空。
> - legacy 作用域列表不再执行 `netsh dhcp server show scope`，而是从一次全局 `dump` 中解析。
...
> - legacy 作用域状态会从全局 `dump` 的 `scope <scopeId> set state` 行解析。
...
> - legacy 同步期间会复用一次全局 `dump` 缓存，解析作用域列表、保留地址、地址范围、排除范围、状态和租期。
...
> - 单作用域刷新会通过 `GET /dhcp/scopes/{scopeId}` 执行 `scope <scopeId> dump`，只解析当前作用域基础信息和缓存数据，不调用 `/dhcp/cache/clear`。
> - 后端同步服务识别到 legacy Agent 后，只按作用域逐个读取租约列表，DHCP 作用域详情采集并发继续遵循「系统配置 / 基础配置 / 同步参数」。
> - legacy Agent 本地会用 runspace worker 池处理 HTTP 请求；DHCP 作用域详情并发由后端同步参数中的 DHCP 作用域并发控制。
...
DHCP 作用域、租约和保留地址操作会转发到对应 DHCP Agent。Agent 执行成功后，后端会等待同一作用域在配置窗口内不再有操作，再创建 `runtime.refresh.dhcp.scope` 局部刷新任务并同步数据库快照。已存在数据库快照的 DHCP 作用域相关操作如果在 Agent 阶段失败，也会标记当前作用域需要延迟局部刷新，用于收敛 DHCP 服务器真实状态。
...
- DNS / DHCP 操作成功后，同一区域或作用域在等待窗口内没有新的操作才会创建局部刷新任务；已存在数据库快照的 DHCP 作用域相关操作在 Agent 阶段失败时，也会按当前作用域进入同一延迟刷新窗口。
...
- `PUT /api/dhcp/scopes/{id}` - 更新 DHCP 作用域
  ...
  - 后端会传递实际变化字段，Agent 只执行对应的名称 / 描述、默认网关、租期、状态或地址范围操作
  - legacy Agent 修改描述时执行 `set comment`，名称变化只执行 `set name`
...
- `PUT /dhcp/scopes/{scopeId}` - 更新 DHCP 作用域名称、描述、租期、状态和地址范围
...
- 单作用域刷新会通过 `GET /dhcp/scopes/{scopeId}` 执行 `scope <scopeId> dump`，再读取当前作用域租约，不调用 `/dhcp/cache/clear`。
