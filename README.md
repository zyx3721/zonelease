...
- **DHCP 管理**：通过 DHCP Agent 管理 Windows DHCP 作用域、租约和保留地址，支持作用域编辑、启停、释放租约、创建 / 编辑 / 删除保留地址，并在成功后按作用域合并局部同步数据库快照。
...
PostgreSQL 保存平台自身数据和资源快照，包括用户、会话、服务器登记、DNS 区域、DNS 记录、DHCP 作用域、租约、保留地址、刷新任务和审计记录。

DNS 区域与 DNS 记录以 PostgreSQL 快照作为页面展示来源。前端访问 `/api/state` 或 `/api/state?includeDns=true` 时，后端都只读取数据库，不会实时遍历 Agent。DNS 资源的标识由后端根据服务器、区域、记录类型、名称和值编码生成，不代表 Windows DNS 内部持久 ID。

Redis 用于刷新事件发布订阅、Redis Stream 最近事件回放、短期运行态缓存和短期分布式锁。后端启动时会连接 Redis，连接失败会终止服务启动；`POST /api/refresh` 会创建后台全量刷新任务，任务执行过程中会发布 `runtime.refresh.all` 进度事件和 `runtime.updated` 事件；`POST /api/dns/zones/{id}/refresh` 会创建指定区域刷新任务，并发布 `runtime.refresh.dns.zone` 事件；DNS / DHCP 管理操作成功后会按同一区域或作用域合并延迟创建局部刷新任务。前端全局布局订阅 `/api/events` 后触发页面重新读取数据库快照，并在全量刷新期间通过独立 toast 展示每个 Agent 的同步进度。自动健康检查、定时全量刷新、刷新任务执行和操作后延迟刷新会使用 Redis 短期锁，避免多实例部署或重复触发时对同一 Agent、DNS 区域或 DHCP 作用域并发执行。刷新任务运行态、Agent 最近健康检查结果和通知未读数会短期缓存到 Redis，接口读取时仍以 PostgreSQL 为最终事实源。
...
│   │   ├── dhcp/                     # Windows DHCP 采集、作用域、租约和保留地址操作
...
│       │   ├── dhcp/                 # DHCP 管理页作用域、排除范围和保留地址创建 / 编辑组件
...
> **注意说明**：使用 legacy Agent 时，推荐先在目标服务器安装并启用 .NET Framework 3.5 以上版本，确保 PowerShell 可加载 `System.Web.Extensions` 处理 JSON 请求体。
>
> - 兼容模式会自动读取 `dhcp-agent/.env` 中的端口、API Key、匿名访问、日志路径和 legacy 请求并发配置。
> - legacy Agent 通过 `netsh dhcp server` 执行 DHCP 作用域、租约和保留地址操作。
> - legacy 作用域注释会从 `show scope` 的注释列解析并保存到作用域描述。
> - legacy 作用域租期会从全局 `dump` 的 `optionvalue 51 DWORD` 秒数按作用域解析；值为 `-1` 时表示无限制。
> - legacy 地址范围会从全局 `dump` 的当前作用域上下文和 `add iprange` 行按作用域解析。
> - legacy 排除范围会从全局 `dump` 的当前作用域上下文和 `add excluderange` 行按作用域解析。
> - 后端保存秒级租期并兼容写入向上取整后的小时数，前端优先按秒动态展示为天 / 时 / 分；无限制时显示为“无限制”。
> - 租约采集会从 `show clients 1` 的 IP 行识别 IPv4、MAC、租用截止日期、`-U-` / `-D-` 等租约类型标记，并从同一行类型标记后提取租约名称。
> - 租用截止日期为 `永不过期` 时，前端按 DHCP 管理控制台显示为 `保留 (活动的)`。
> - legacy 同步期间会复用一次全局 `dump` 缓存，解析保留地址、地址范围和排除范围。
> - 保留地址采集会从全局 `dump` 的 `reservedip` 命令行提取 IP、MAC、名称和描述。
> - `GET /dhcp/probe` 用于后端测试 DHCP Agent 连接，不枚举作用域，也不会触发全局 `dump`。
> - `GET /dhcp/scopes` 用于同步作用域列表，会解析租期并预热本次同步的全局 `dump` 缓存。
> - 后端同步服务识别到 legacy Agent 后，会按作用域逐个读取租约和保留地址，DHCP 作用域详情采集并发继续遵循「系统配置 / 基础配置 / 同步参数」。
> - legacy Agent 本地会用 runspace worker 池并发处理 HTTP 请求，并发上限由 `DHCP_AGENT_LEGACY_REQUEST_CONCURRENCY` 控制。
> - legacy 粒度同步会先写入作用域列表，再在每个作用域详情采集成功后立即写入该作用域快照，避免全量任务超时时页面仍然没有任何已完成数据。
> - Windows Server 2008/2008 R2 上建议先从较低并发观察 Agent 响应，作用域或租约较多导致总耗时超限时优先调大 Agent 全量同步超时。
...
- **保存**：只有当前 Agent 地址、角色、API Key 和 TLS 设置测试成功后才可点击；保存时只检查 Agent 名称和接口地址唯一性，不再重复访问 Agent，保存成功后显示固定同步 toast 并自动同步该 Agent。
...
- 顶部工具栏手动全量刷新会同步所有已登记 DNS / DHCP 服务器。
- 后端 `RUNTIME_DEEP_SYNC_INTERVAL` 定时任务默认关闭，设为大于 `0` 的 Go duration 后会周期性执行全量同步。
...
- 新增 DNS 区域成功后，DNS Agent 会先确认 Windows DNS 中已能读取该区域，后端会立即读取该区域的默认 SOA、NS 等记录并写入快照，同时创建 `runtime.refresh.dns.zone` 任务用于最终收敛。
...
- DNS 记录类型使用不同颜色标识，便于快速区分 A、AAAA、CNAME、MX、TXT 等记录
- 正向区域仅支持编辑 / 删除 A 和 CNAME 记录；反向区域仅支持编辑 / 删除 PTR 记录，其他记录类型会禁用操作按钮并显示对应提示
- 新建记录前，后端会基于数据库快照校验同名同类型同值重复记录和 CNAME 互斥规则
...
DHCP 作用域、租约和保留地址操作会转发到对应 DHCP Agent。Agent 执行成功后，后端会等待同一作用域在配置窗口内不再有操作，再创建 `runtime.refresh.dhcp.scope` 局部刷新任务并同步数据库快照。

DHCP 作用域、租约、保留地址字段、Agent 接口、数据库快照策略和当前操作边界详情见 [docs/dhcp-management.md](docs/dhcp-management.md)。
...
- 写入 `refresh_tasks` 记录
- 后台同步所有已登记服务器的 DNS / DHCP 数据到 PostgreSQL
...
- 进度来自 `refresh_tasks.payload` 中的 `totalAgents`、`startedAgents`、`syncedAgents`、`failedAgents`、`currentAgent` 和 `agentEvent` 字段。
- 任务完成或失败后会保留最终进度快照，并写入 `startedAt`、`finishedAt` 和 `agentResults` 便于排查。
- 前端收到 SSE 后只重新读取数据库快照和更新 toast，不会再弹出“刷新事件已同步”类提示。

DNS 区域卡片右侧刷新按钮会调用 `POST /api/dns/zones/{id}/refresh` 创建区域刷新任务。后端只访问该区域对应 DNS Agent 的记录接口，并用最新记录替换数据库中该区域的记录快照。

前端全局布局订阅 `/api/events`。收到 `runtime.refresh.all`、`runtime.refresh.server`、`runtime.refresh.dns.zone`、`runtime.refresh.dhcp.scope` 或 `runtime.updated` 事件后，会触发页面重新读取数据库快照。

全量同步、DNS 区域和 DHCP 作用域采集并发，以及操作后局部刷新等待时间从「系统配置 / 基础配置 / 同步参数」读取并入库保存。Agent 测试、同步和 DNS / DHCP 操作超时时间从「系统配置 / 基础配置 / Agent 判定」读取，默认 20 秒；后端自动健康检查的间隔和并发数也从该配置读取，默认每 1 分钟串行检查。各刷新入口是否访问 Agent、刷新范围、任务类型、Redis/SSE 事件和配置边界详情见 [docs/refresh-sync.md](docs/refresh-sync.md)。

## 6.6 系统配置

「系统配置」页面参考配置中心布局，包含：

- **基础配置**：维护站点名称、登录展示、控制台品牌、找回密码安全时效、同步并发、操作后刷新等待、Agent 离线判定次数、Agent 操作超时、自动健康检查间隔和自动检查并发。
...
- 点击「测试」会弹出测试邮箱输入窗口，并调用 `POST /api/settings/notifications/email/test` 发送测试邮件。
- 当前界面不展示邮件模板预览入口。
...
- 点击通知按钮会打开消息面板，支持查看最近消息、标记全部已读和清空。
- 点击单条通知会标记已读并跳转到「操作审计」页面辅助排查。
...
| `RUNTIME_DEEP_SYNC_INTERVAL` | `0`                     | 后端定时全量同步间隔，默认关闭；设为大于 `0` 的 Go duration 后启用 |
...
- DNS / DHCP 操作成功后，同一区域或作用域在等待窗口内没有新的操作才会创建局部刷新任务。
- Agent 离线失败次数、Agent 操作超时时间、Agent 全量同步超时时间、自动健康检查间隔和自动检查并发由「系统配置 / 基础配置 / Agent 判定」控制。
- 后台同步和自动连通性检查连续失败达到阈值后才标记为 `Offline` 并创建 Agent 离线通知。
- 仪表板或设置页手动测试连接失败会立即标记为 `Offline` 并创建通知。
- 成功同步或健康检查会清零失败计数并更新最近检查时间。
- Agent 操作超时默认 `20` 秒，可配置 `1` 到 `60` 秒，覆盖 Agent 测试、DNS 区域和记录操作、DHCP 作用域、租约和保留地址操作。
- Agent 全量同步超时默认 `300` 秒，可配置 `60` 到 `600` 秒，覆盖全量同步、单 Agent 同步、DNS 区域同步和 DHCP 作用域同步。
...
| `DNS_AGENT_API_KEY`                    | 空          | DNS Agent API Key                                                                                          |
...
| `DHCP_AGENT_API_KEY`                    | 空          | DHCP Agent API Key                                                                       |
| `DHCP_AGENT_ALLOW_ANONYMOUS`            | `false`     | 是否允许匿名访问非健康检查接口                                                           |
| `DHCP_AGENT_POWERSHELL_TIMEOUT_SECONDS` | `180`       | Go DHCP Agent 单次 PowerShell 命令超时，覆盖作用域、租约和保留地址读取、创建、删除等操作 |
| `DHCP_AGENT_LOG_PATH`                   | `agent.log` | DHCP Agent 日志路径，Go Agent 和 legacy Agent 都会将运行日志写入该文件                   |
| `DHCP_AGENT_LEGACY_REQUEST_CONCURRENCY` | `5`         | legacy DHCP Agent 本地 HTTP 请求并发，范围 1 到 50，控制同时处理多少个作用域详情请求      |

# 七、安全说明

- 生产环境必须设置固定且足够随机的 `JWT_SECRET`，避免服务重启导致会话失效或 Token 可被伪造。
- Agent API Key 必须使用强随机值，并仅通过内网或 HTTPS 传输。
...
- DHCP 作用域、租约和保留地址变更会先转发到 DHCP Agent，Agent 成功后才更新或按作用域合并局部同步 PostgreSQL 快照。
...
以下接口除 `POST /api/auth/login`、`GET /api/auth/providers`、找回密码相关公开接口、`GET /api/public/base`、`GET /api/events` 和 `GET /api/health` 健康检查外，均需要在请求头中携带 `Authorization: Bearer <token>`。

## 10.1 认证

- `POST /api/auth/login` - 登录，返回会话 Token、用户信息、最长过期时间和最近活跃时间；用户名或密码为空会返回 `invalid_login`
- `GET /api/auth/providers` - 获取公开认证方式，登录页用于读取已启用的本地或 AD/LDAP 登录方式
- `POST /api/auth/logout` - 注销当前会话；后端会删除当前 Bearer Token 对应会话
...
- `POST /api/dhcp/scopes` - 创建 DHCP 作用域
  - 请求字段包含 `name`、`description`、`subnet`、`startRange`、`endRange`、`leaseDurationHours`、`leaseDurationSeconds`、`state` 和 `serverId`
  - `name`、`subnet`、`serverId` 必填
...
- `PUT /api/dhcp/scopes/{id}` - 更新 DHCP 作用域
  - 当前支持更新名称、租期、状态和地址范围；子网仍以 Agent 同步快照为准
...
- `POST /api/dhcp/scopes/{id}/refresh` - 刷新指定 DHCP 作用域
  - 后端创建 `runtime.refresh.dhcp.scope` 局部刷新任务
  - 仅同步当前作用域的排除范围、租约和保留地址快照
...
- `POST /api/dns/zones` - 创建 DNS 区域
  - 请求字段包含 `name`、`type`、`reverse`、`dynamicUpdate` 和 `serverId`
  - `name` 和 `serverId` 必填
...
- `POST /api/dns/zones/{id}/refresh` - 刷新指定 DNS 区域记录
  - 后端创建 `runtime.refresh.dns.zone` 刷新任务
...
- `PUT /api/dns/records/{id}` - 编辑 DNS 记录值
  ...
  - 仅允许修改记录值和 A / AAAA 记录的 `createPtr` 标记，名称、类型、TTL 和所属区域不变
  ...
  - A / AAAA 记录编辑窗口会展示“更新相关的指针 PTR 记录”配置项；A 记录带 `createPtr` 标记时，会同步维护旧值和新值对应的 PTR 快照，并在响应中通过 `relatedRecords` 返回新 PTR 记录
...
- `GET /api/notifications?limit=20` - 获取右上角通知中心消息，仅返回 Agent 异常和平台基础服务异常等非刷新任务消息
...
- `GET /api/settings/base` - 获取系统基础配置
  - 返回站点名称、登录展示、控制台品牌、安全时效、同步并发、操作后刷新等待、Agent 离线判定次数、Agent 操作超时、Agent 全量同步超时、自动健康检查间隔和自动检查并发
...
- `PUT /api/settings/base` - 保存系统基础配置
  ...
  - `agentOperationTimeoutSeconds` 表示 Agent 测试、同步和 DNS / DHCP 操作超时时间，范围 1 到 60 秒，默认 20 秒
  ...
  - 保存后找回密码安全时效、同步并发、操作后刷新等待、Agent 离线判定次数、Agent 操作超时、Agent 全量同步超时、自动健康检查间隔和自动检查并发会被运行逻辑读取
...
- `GET /api/settings/permissions` - 获取可分配权限列表
  ...
  - 后端对 DNS、DHCP、Agent、刷新和系统配置写接口执行权限校验，前端会按权限隐藏对应操作入口
...
- `POST /api/refresh` - 创建刷新任务并发布 SSE 事件
  ...
  - 后端写入 `refresh_tasks` 记录，后台同步所有已登记服务器，并写入 `Queued refresh` 审计记录
  - 全量刷新执行期间和完成 / 失败后会在 payload 中写入 `totalAgents`、`startedAgents`、`syncedAgents`、`failedAgents`、`currentAgent`、`agentEvent`、`startedAt`、`finishedAt` 和 `agentResults`
  - 刷新范围、任务状态、Redis/SSE 事件和定时刷新规则详见 [docs/refresh-sync.md](docs/refresh-sync.md)
- `GET /api/refresh/tasks?limit=all` - 获取刷新任务列表，`limit` 默认 30；传 `all` 时返回全部任务
...
- `POST /api/servers` - 添加 Windows DNS / DHCP 服务器登记
  ...
  - 校验通过后以 `Online` 状态写入服务器登记，前端随后自动创建单 Agent 同步任务
  - 创建成功后写入 `Created server` 审计记录
- `POST /api/servers/probe` - 测试未保存的 Agent 连接
  - 请求字段包含 `name`、`host`、`role`、`agentUrl`、`apiKey` 和 `tlsInsecure`
  - 调用请求体中的 Agent `/health` 和角色对应的受保护业务接口，不写入服务器表和审计记录
- `DELETE /api/servers/{id}` - 删除服务器登记
  - 服务器不存在时返回 `delete_server_failed`
  - 删除成功后会清理该 Agent 的离线通知和健康检查运行态缓存
  - 删除成功后写入 `Deleted server` 审计记录
- `POST /api/servers/{id}/ping` - 调用已登记 Agent 的 `/health` 和角色对应业务接口并更新服务器状态；手动检查失败会立即标记为 `Offline`
...
- `GET /api/state` - 获取控制台状态
  - 返回服务器、DNS 区域、DNS 记录、DHCP 作用域、租约、保留地址和审计记录
  - 默认只读取 PostgreSQL 快照，不会实时读取 DNS Agent
- `GET /api/state?includeDns=true` - 获取控制台状态
  - `includeDns` 为兼容参数，当前不会触发 Agent 实时读取
...
Agent 除 `GET /health` 外，所有业务接口在非匿名模式下都需要携带 `X-API-Key: <AGENT_API_KEY>`。响应统一采用 envelope：`success` 表示是否成功，`data` 承载业务数据，`error` 承载错误对象，`requestId` 用于排查日志。
...
- 后端同步服务识别到 `/health` 返回 `mode=legacy` 后，会改用逐作用域接口采集，并发遵循系统配置中的 DHCP 作用域并发。
- legacy Agent 本地通过 runspace worker 池并发处理这些 HTTP 请求，并发上限由 `DHCP_AGENT_LEGACY_REQUEST_CONCURRENCY` 控制。
- 作用域列表会先入库。
- 每个作用域详情优先通过 `GET /dhcp/scopes/{scopeId}/details` 一次读取排除范围、租约和保留地址。
- `GET /dhcp/scopes/{scopeId}/details` 内部会并行读取同一作用域下的排除范围、租约和保留地址，成功后立即写入该作用域快照。
- 旧脚本返回 `404` 时自动回退到排除范围、租约和保留地址接口。
- 后端在 DHCP 同步结束后会调用 legacy Agent 清理本次同步使用的全局 dump 缓存。
- legacy 日志会记录 scopes、leases、reservations 和 scope details 的 start/done 阶段，便于定位卡住的作用域。
